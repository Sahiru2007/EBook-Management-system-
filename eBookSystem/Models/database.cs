...
namespace eBookSystem.Models
{
    ...
    {
        private readonly string _connectionString = "Data Source=DAPHABDEPT1\\SQLEXPRESS;Initial Catalog=eBookSystemDB;Integrated Security=True;TrustServerCertificate=True";
    ...
    public void UpdateBookAverageRating(int bookId, double? averageRating)
    {
        ...
        public void InsertOrder(Order order)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("INSERT INTO Orders (UserId, FullName, Address, City, State, ZipCode, Country, TotalAmount, OrderDate, Status) VALUES (@UserId, @FullName, @Address, @City, @State, @ZipCode, @Country, @TotalAmount, @OrderDate, @Status)", con);
                    ...
                    cmd = new SqlCommand("SELECT TOP 1 Id FROM Orders ORDER BY Id DESC", con);
        ...
        public void InsertOrderItem(int orderId, int bookId, int quantity)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("INSERT INTO OrderItems (OrderId, BookId, Quantity) VALUES (@OrderId, @BookId, @Quantity)", con);
        ...
        public Order GetLatestOrderByUserId(string userId)
        {
            ...
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT TOP 1 * FROM Orders WHERE UserId = @UserId ORDER BY Id DESC", con);
                    cmd.Parameters.AddWithValue("@UserId", userId);
                    SqlDataReader reader = cmd.ExecuteReader();
